...
这是一个面向软件课程设计的本机 B/S Web 应用，完成注册/登录、创建或加入家庭、家长与普通成员权限、成员与分类管理、收支 CRUD、家庭预算、周期流水、CSV 导入导出、操作日志和家庭月报统计。项目不包含通知、附件、多家庭、银行同步、支付接口或公网部署。
...
6. 设置家庭总预算和分类预算，确认正常、接近上限、超支状态随流水实时变化。
7. 创建工资、房租等周期模板，对同一月份连续生成两次，确认不会产生重复流水。
8. 导出当前筛选结果，修改 CSV 后先预览再提交导入，确认错误行不会写入。
9. 查看环比、储蓄率、异常支出提示和家长专属操作日志。
10. 停用分类或成员后确认历史记录仍保留；尝试停用最后一名家长，确认系统拒绝。
11. 退出、重新登录，并在手机宽度完成一次新增收支。
...
本项目服务课程演示，操作日志只覆盖本系统内的关键业务变更，不等同于生产合规审计；系统也不承诺高可用、银行对账、投资分析或公网安全运维。报告素材必须来自实际源码、测试和界面，不填写虚假的多人分工。
