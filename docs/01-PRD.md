...
当前验收版本实现注册登录、创建或加入家庭、角色权限、成员管理、分类管理、收支管理、家庭预算、周期流水、CSV 导入导出、操作日志和增强统计。不实现通知、附件、多家庭切换、公网部署、银行/支付平台连接和投资分析。
...
├── 增强：家庭总预算与支出分类预算
├── 增强：周期流水模板与按月幂等生成
├── 增强：CSV 预览、原子导入与筛选结果导出
├── 增强：环比、储蓄率、异常支出和家长操作日志
├── 重要：个人资料与密码修改
└── 不在当前版本：提醒、附件、多家庭、公网部署
```
...
| `/budgets` | 家庭预算 | 已入户 | 预算执行；仅家长可设置总额和分类额度 |
| `/recurring` | 周期流水 | 已入户 | 模板 CRUD、启停和按月生成 |
| `/audit-logs` | 操作日志 | 家长 | 关键业务变更的时间线与分页 |
...
- 预算按自然月保存，总预算和每个支出分类预算在同月内唯一；使用率达到 80% 提醒，超过 100% 标记超支。
- 周期模板支持收入或支出、归属成员、分类、金额、每月发生日和备注；同一模板同一月份最多生成一笔，月底日期自动收敛。
- CSV 固定列为“发生日期、类型、分类、成员编号、金额、备注”，单次最多 500 行；预览有任何错误时整批不写入。
- 增强统计返回等长上期环比、储蓄率、预算执行和支出异常；异常需至少有连续三个月基线。
- 家长可查看成员、分类、流水、预算、周期模板、邀请码等关键变更日志；普通成员无权读取。

## 7. 视觉与文案规范

- 采用“中文编辑月报”视觉：米白纸张、墨色标题、朱砂重点、竹青收入、群青结余，配合月份竖栏与细线分隔形成记忆点。
- 卡片和控件使用克制的小圆角，不依赖外部字体、外链图片或视频；登录页允许使用仓库内的蓝纸账本背景资源。
