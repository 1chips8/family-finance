...
async function submit() {
  error.value = ''
  if (active.value === 'create' && !createForm.name.trim()) { error.value = '请给家庭起一个名字'; return }
  if (active.value === 'join' && !/^[A-Za-z0-9]{8}$/.test(joinForm.inviteCode)) { error.value = '请输入8位邀请码'; return }
  loading.value = true
  try {
...
<template>
  <main class="onboarding-page">
    ...
    <section class="onboarding-panel">
      <div class="segmented"><button :class="{ active: active === 'create' }" @click="active = 'create'; error = ''">创建家庭</button><button :class="{ active: active === 'join' }" @click="active = 'join'; error = ''">输入邀请码</button></div>
      <form class="form-stack" @submit.prevent="submit">
        <div v-if="active === 'create'" class="field"><label for="household-name">家庭名称</label><el-input id="household-name" v-model="createForm.name" size="large" placeholder="例如：小林一家" maxlength="80" show-word-limit /></div>
        <div v-else class="field"><label for="invite-code">家庭邀请码</label><el-input id="invite-code" v-model="joinForm.inviteCode" size="large" placeholder="8位大写字母或数字" maxlength="8" @input="joinForm.inviteCode = joinForm.inviteCode.toUpperCase()" /></div>
